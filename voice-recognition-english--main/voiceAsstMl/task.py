...
def doTranslate():
    Spk.say("from which language you want to translate?")
    fromLang = takeCommand()
    Spk.say("in which language you want to taranslate?")
    toLang = takeCommand()
    Spk.say("tell me the sentence that you want to translate?")
...
def openApp(query):
    ...
    if "cmd" in query:
        os.startfile(
            "C://Users//Sdewa//AppData//Roaming//Microsoft//Windows//Start Menu//Programs//System Tools//Command Prompt.lnk")
        Spk.say("done sir...")
    if "control panel" in query:
        os.startfile(
            "C://Users//Sdewa//AppData//Roaming//Microsoft//Windows//Start Menu//Programs//System Tools//Control Panel.lnk")
        Spk.say("done sir....")
...
def InputFn(reply, query):
    if "wikipedia" in reply:
        query = str(query).replace("who is", "").replace("how to", "").replace("what is", "").replace("about",
                                                                                                      "").replace(
            "according to", "").replace("wikipedia", "").replace("according to wikipedia", "")
        try:
        ...
        query = query.replace("in google", "").replace("on", "").replace("google", "").replace("search", "").replace(
            "google search", "").replace("according to google", "").replace("accroding to", "")
        ...
        query = str(query).replace("in youtube", "").replace("on youtube", "").replace("youtube search", "").replace(
            "search on youtube", "")
...
def scrap(sentence3):
    ...
    try:
        url = "https://www.google.com/search?q=" + sentence3
